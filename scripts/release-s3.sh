...
if [[ "$CHANNEL" == "nightly" ]]; then
  ...
  today=$(date +"%F")
  echo "Uploading all artifacts to s3://packages.timber.io/vector/tokio-compat/$today"
  aws s3 cp "$td" "s3://packages.timber.io/vector/tokio-compat/$today" --recursive --sse --acl public-read
